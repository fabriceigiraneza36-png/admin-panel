...
const statusVariant = (s) => ({
  ...
  completed: 'info', needs_info: 'warning',
})[s] || 'default'
...
// ── JsonListEditor — edits simple string arrays inline ───────────────────────
...
function ItineraryEditor({ value = [], onChange }) {
  ...
  const upd = (i, k, v) =>
    onChange(value.map((d, j) => j === i ? { ...d, [k]: v } : d))
...
function PricingTierEditor({ value = [], onChange, currency = 'USD' }) {
  const add = () =>
    onChange([...value, { label: '', price: '', description: '' }])
  const upd = (i, k, v) =>
    onChange(value.map((t, j) => j === i ? { ...t, [k]: v } : t))
  const remove = (i) => onChange(value.filter((_, j) => j !== i))
  ...
  return (
    <div className="input-group">
      ...
      <div className="space-y-2">
        {value.map((tier, i) => (
          <div key={i} className="grid grid-cols-3 gap-2 items-start border border-slate-200 rounded-xl p-3 bg-slate-50">
            ...
            <div className="flex gap-1">
              <input className="input text-sm flex-1" placeholder="Description"
                value={tier.description} onChange={e => upd(i, 'description', e.target.value)} />
              <button type="button" onClick={() => remove(i)}
...
function InfoFieldBuilder({ value = [], onChange }) {
  ...
  const upd = (i, k, v) =>
    onChange(value.map((f, j) => j === i ? { ...f, [k]: v } : f))
  const remove = (i) => onChange(value.filter((_, j) => j !== i))
  ...
  return (
    <div>
      ...
      <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
        {value.map((field, i) => (
          <div key={field.id || i}
            className="border border-slate-200 rounded-xl p-3 bg-gradient-to-r from-slate-50 to-white">
            ...
            {['select','radio'].includes(field.type) && (
              <div>
                ...
                <input className="input text-sm"
                  ...
                  onChange={e =>
                    upd(i, 'options', e.target.value.split(',').map(o => o.trim()).filter(Boolean))
                  } />
              </div>
            )}
          </div>
        ...
        {!value.length && (
          <div className="text-center py-6 border-2 border-dashed border-slate-200 rounded-xl">
            ...
            <p className="text-sm text-slate-400">No fields yet. Click "Add Field" above.</p>
          </div>
        )}
      </div>
    </div>
      ...
      >
        ...
        <div className="text-right shrink-0">
          <p className="text-sm font-bold text-emerald-600">
            {booking.currency} {formatNumber(booking.total_price || 0)}
          </p>
      ...
      {expanded && (
        <div className="border-t border-slate-100 p-3 bg-slate-50 space-y-3">
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div><span className="text-slate-400">Travel Date:</span> <span className="font-medium">{booking.travel_date || '—'}</span></div>
            <div><span className="text-slate-400">Travelers:</span> <span className="font-medium">{booking.travelers_count || booking.adults}</span></div>
            <div><span className="text-slate-400">Phone:</span> <span className="font-medium">{booking.guest_phone || '—'}</span></div>
            <div><span className="text-slate-400">Priority:</span> <span className="font-medium capitalize">{booking.priority}</span></div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {booking.status === 'pending' && (
              <button onClick={() => onConfirm(booking)} className="btn-sm bg-emerald-500 text-white hover:bg-emerald-600 rounded-lg px-3 py-1.5 text-xs font-semibold flex items-center gap-1">
                <CheckCircle size={12} /> Confirm
              </button>
            )}
            {['pending','needs_info','confirmed'].includes(booking.status) && (
              <button onClick={() => onCancel(booking)} className="btn-sm bg-red-500 text-white hover:bg-red-600 rounded-lg px-3 py-1.5 text-xs font-semibold flex items-center gap-1">
                <XCircle size={12} /> Cancel
              </button>
            )}
            <button onClick={() => onInfoRequest(booking)} className="btn-secondary btn-sm text-xs flex items-center gap-1">
...
export default function Packages() {
  ...
  const viewModal   = useModal()   // package detail + tabs
  const formModal   = useModal()   // create / edit package
  const deleteModal = useModal()   // confirm delete
  const replyModal  = useModal()   // admin reply to a message
  const infoModal   = useModal()   // create info-request form
  ...
  const [activeTab, setActiveTab] = useState('details') // details|messages|bookings|info

  // Detail view sub-data
  ...
  const [replyTarget,   setReplyTarget]   = useState(null) // { user_id, parent_id }
  const [replySending,  setReplySending]  = useState(false)

  // Info request form
  const [infoForm,  setInfoForm]  = useState(INIT_INFO_REQUEST)
  const [infoTarget, setInfoTarget] = useState(null) // booking or message context
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await packagesAPI.getAll({ ...params, admin: true })
      // Fallback: try admin/all if getAll doesn't support admin flag
      const rows = data.data || data.packages || []
      setItems(rows)
      pag.setTotal(data.pagination?.total || data.total || 0)
    } catch (e) { toast.error(getErrorMessage(e)) }
    finally { setLoading(false) }
  }, [pag.page, pag.limit, sortBy, sortOrder, dSearch, pubFilter, catFilter])

  useEffect(() => { load() }, [load])

  // ── Load sub-data when viewing a package ──────────────────────────────────
  ...
  const loadSubData = useCallback(async (pkg, tab) => {
    ...
    try {
      if (tab === 'messages') {
        ...
        setPkgMessages(data.data || [])
      } else if (tab === 'bookings') {
        const { data } = await packagesAPI.getBookings(pkg.id)
        setPkgBookings(data.data || [])
      } else if (tab === 'info') {
        const { data } = await packagesAPI.getInfoRequests(pkg.id)
        setPkgInfoReqs(data.data || [])
      }
    } catch (e) { toast.error(getErrorMessage(e)) }
  ...
  const openView = (pkg) => {
    ...
    setPkgMessages([]); setPkgBookings([]); setPkgInfoReqs([])
  }
  ...
  const openCreate = () => {
    setForm(INIT_PACKAGE); setEditing(null)
  ...
  const openEdit = (p) => {
    ...
    ;['pricing_tiers','images','gallery','features','inclusions','exclusions',
      ...
      if (typeof f[k] === 'string') { try { f[k] = JSON.parse(f[k]) } catch { f[k] = [] } }
    })
  ...
  const handleTogglePublish = async (pkg) => {
    try {
      if (pkg.is_published) {
        await packagesAPI.unpublish(pkg.id); toast.success('Package unpublished')
      } else {
        await packagesAPI.publish(pkg.id); toast.success('Package published')
      }
      load()
      if (viewModal.data?.id === pkg.id) {
  ...
  const handleSendReply = async () => {
    if (!replyBody.trim()) return toast.error('Message required')
  ...
  const openInfoRequest = (context = null) => {
    setInfoForm(INIT_INFO_REQUEST)
    setInfoTarget(context)
    if (context?.booking_id) {
      setInfoForm(p => ({
        ...p,
        booking_id: context.booking_id,
        user_id: context.user_id || '',
        target_email: context.guest_email || '',
        target_name: context.guest_name || '',
      }))
    }
    infoModal.open(viewModal.data)
  }

  const infoUpd = (k, v) => setInfoForm(p => ({ ...p, [k]: v }))

  const handleSendInfoRequest = async () => {
    if (!infoForm.title?.trim()) return toast.error('Title required')
    if (!infoForm.fields?.length) return toast.error('Add at least one field')
    setSaving(true)
    try {
  ...
  const columns = [
    {
      ...
      render: (_, r) => (
        <div className="flex items-center gap-3">
          <Avatar src={r.thumbnail_url || r.cover_image_url} name={r.title} size="sm" rounded="lg" />
          <div>
            <p className="font-semibold text-slate-800 max-w-[180px] truncate">{r.title}</p>
            <p className="text-xs text-slate-400">{r.category || '—'} · {r.destination || r.country || '—'}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (v, r) => (
        <div className="text-right">
          <p className="font-bold text-emerald-700">{r.currency} {formatNumber(v || 0)}</p>
    ...
    },
    {
      ...
      render: (v, r) => v ? `${v}D / ${r.duration_nights || v - 1}N` : '—',
    },
    {
  ...
  const Section = ({ id, title, icon: Icon_, children }) => (
    <div className="border border-slate-200 rounded-2xl overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-slate-700">
          {Icon_ && <Icon_ size={16} className="text-emerald-600" />}
          {title}
        </span>
          ...
          <div key={msg.id}
            ...
            <div className="flex items-start gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                msg.sender_type === 'admin' ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-600'
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-400 ml-auto">{formatTimeAgo(msg.created_at)}</span>
                </div>
                <p className="text-sm text-slate-700 whitespace-pre-wrap">{msg.body}</p>

                {/* Info request embedded form preview */}
                {msg.message_type === 'info_request' && msg.info_request_fields && (
                  <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                    <span className="font-semibold">📋 Info Request Form</span>
                    {' '}— {msg.info_request_fields?.length || 0} fields ·{' '}
                    <span className="capitalize">{msg.info_request_status}</span>
                  </div>
                )}
              </div>
            </div>
            {msg.sender_type !== 'admin' && (
              <div className="mt-2 flex justify-end">
                <button onClick={() => openReply(msg)} className="btn-secondary btn-sm text-xs">
                  <Send size={11} /> Reply
                </button>
              </div>
            )}
  ...
  const TabBookings = () => (
    <div className="space-y-3">
      <h4 className="font-semibold text-slate-700">Bookings ({pkgBookings.length})</h4>
      {subLoading && (
        <div className="flex justify-center py-8">
          <div className="w-6 h-6 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />
        </div>
  ...
  const TabInfoRequests = () => (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="font-semibold text-slate-700">Info Requests ({pkgInfoReqs.length})</h4>
      ...
      {!subLoading && !pkgInfoReqs.length && (
        <div className="text-center py-8 text-slate-400 text-sm">No info requests sent yet.</div>
      )}
      <div className="space-y-3 max-h-[420px] overflow-y-auto">
        {pkgInfoReqs.map(ir => (
          <div key={ir.id} className="border border-slate-200 rounded-xl p-4 bg-white">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-semibold text-slate-800">{ir.title}</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  To: {ir.target_name || ir.target_email || 'All'} ·{' '}
                  {ir.fields?.length || 0} fields · {formatTimeAgo(ir.created_at)}
                </p>
              </div>
              <Badge
            ...
            {ir.status === 'responded' && ir.response && (
              <div className="mt-3 p-3 bg-emerald-50 border border-emerald-100 rounded-lg">
                <p className="text-xs font-semibold text-emerald-700 mb-1">Response received:</p>
                <div className="space-y-1">
                  {Object.entries(ir.response).map(([k, v]) => (
    ...
    <div className="space-y-5 page-enter">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage travel packages ({pag.total} total)</p>
        </div>
      ...
      <div className="card p-4">
        <FilterBar>
          <SearchBar value={search} onChange={setSearch} placeholder="Search packages…" className="max-w-sm" />
          <FilterSelect
            label="Status" value={pubFilter}
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          VIEW MODAL — Tabbed: Details | Messages | Bookings | Info Requests
          ══════════════════════════════════════════════════════════════════ */}
      <Modal
          ...
          <div className="flex justify-between gap-2 flex-wrap">
            <div className="flex gap-2">
              <button
                ...
                className={viewModal.data?.is_published ? 'btn-warning btn-sm' : 'btn-success btn-sm'}
              >
                {viewModal.data?.is_published
                  ? <><EyeOff size={13} /> Unpublish</>
                  : <><Globe2 size={13} /> Publish</>}
              </button>
              <button onClick={() => { viewModal.close(); openEdit(viewModal.data) }} className="btn-primary btn-sm">
                <Pencil size={13} /> Edit
              </button>
            </div>
            <button onClick={viewModal.close} className="btn-secondary btn-sm">Close</button>
          </div>
        }
      >
        {viewModal.data && (
              ...
              <div className="space-y-4">
                <ModalSection title="Overview">
                  <ModalGrid>
                    ...
                    <ModalField label="Duration"    value={
                      viewModal.data.duration_days
                        ? `${viewModal.data.duration_days} days / ${viewModal.data.duration_nights || ''} nights`
                        : '—'
                    } />
                    <ModalField label="Price" value={
                      <span className="font-bold text-emerald-600">
                        {viewModal.data.currency} {formatNumber(viewModal.data.price)}
                        {' '}<span className="text-slate-400 font-normal text-xs">{viewModal.data.price_label}</span>
                      </span>
                    } />
                    <ModalField label="Max Travelers" value={viewModal.data.max_travelers || '—'} />
                ...
                {viewModal.data.short_description && (
                  <ModalField label="Short Description" value={viewModal.data.short_description} />
                )}

                {viewModal.data.features?.length > 0 && (
                  <ModalSection title="Features">
                    <div className="flex flex-wrap gap-2">
                      {(typeof viewModal.data.features === 'string'
                        ? JSON.parse(viewModal.data.features)
                        : viewModal.data.features
                      ).map((f, i) => (
                ...
                {viewModal.data.pricing_tiers?.length > 0 && (
                  <ModalSection title="Pricing Tiers">
                    <div className="space-y-2">
                      {(typeof viewModal.data.pricing_tiers === 'string'
                        ? JSON.parse(viewModal.data.pricing_tiers)
                        : viewModal.data.pricing_tiers
                      ).map((t, i) => (
                        <div key={i} className="flex items-center justify-between bg-slate-50 px-3 py-2 rounded-lg">
                          <span className="font-medium text-sm">{t.label}</span>
                          <span className="font-bold text-emerald-600">{viewModal.data.currency} {formatNumber(t.price)}</span>
                        </div>
                      ))}
                    </div>
                  </ModalSection>
      ...
      >
        <div className="space-y-4">
          ...
          {/* ── Basic Info ── */}
          <Section id="basic" title="Basic Info" icon_={Package}>
            <ModalGrid>
              <div className="input-group sm:col-span-2">
                <label className="input-label">Title *</label>
                <input className="input" value={form.title}
                  onChange={e => upd('title', e.target.value)} placeholder="Package title" />
              </div>
              <div className="input-group">
              ...
              <div className="input-group">
                ...
                <input className="input" value={form.destination}
                  onChange={e => upd('destination', e.target.value)} placeholder="e.g. Serengeti" />
              </div>
              <div className="input-group">
                <label className="input-label">Country</label>
                <input className="input" value={form.country}
                  onChange={e => upd('country', e.target.value)} placeholder="e.g. Tanzania" />
              </div>
            </ModalGrid>
            <div className="input-group">
              <label className="input-label">Short Description</label>
              <textarea className="input min-h-[70px]" value={form.short_description}
            ...
            <ModalGrid>
              ...
              <div className="input-group">
                ...
                <input className="input" value={form.price_label}
                  onChange={e => upd('price_label', e.target.value)} placeholder="per person" />
              </div>
              <div className="input-group">
          ...
          {/* ── Duration & Capacity ── */}
          <Section id="duration" title="Duration & Capacity" icon_={Calendar}>
            <ModalGrid>
              <div className="input-group">
          ...
          {/* ── Media ── */}
          <Section id="media" title="Media" icon_={Image}>
            <ModalGrid>
              <ImageUpload label="Thumbnail" value={form.thumbnail_url}
                onChange={v => upd('thumbnail_url', v)} folder="packages" />
              <ImageUpload label="Cover Image" value={form.cover_image_url}
                onChange={v => upd('cover_image_url', v)} folder="packages" />
            </ModalGrid>
            <div className="input-group">
          ...
          {/* ── Package Details ── */}
          <Section id="pkgdetails" title="Package Details" icon_={FileText}>
            <JsonListEditor
            ...
            />
            <ItineraryEditor value={form.itinerary} onChange={v => upd('itinerary', v)} />
            <TagInput label="Tags" value={form.tags} onChange={v => upd('tags', v)} />
          </Section>

          {/* ── Card Design ── */}
          <Section id="design" title="Card Design" icon_={Palette}>
            <ModalGrid>
              <div className="input-group">
              ...
              <div className="input-group">
                ...
                <div className="flex gap-2">
                  ...
                  <input className="input flex-1" value={form.accent_color}
                    onChange={e => upd('accent_color', e.target.value)} placeholder="#047857" />
                </div>
              </div>
              <div className="input-group">
          ...
          {/* ── SEO & Options ── */}
          <Section id="seo" title="SEO & Options" icon_={Globe2}>
            <ModalGrid>
              <div className="input-group">
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <button onClick={replyModal.close} className="btn-secondary" disabled={replySending}>
              Cancel
            </button>
            <button onClick={handleSendReply} className="btn-primary" disabled={replySending}>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          INFO REQUEST MODAL — build dynamic form to send to user
          ══════════════════════════════════════════════════════════════════ */}
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <button onClick={infoModal.close} className="btn-secondary" disabled={saving}>Cancel</button>
            <button onClick={handleSendInfoRequest} className="btn-primary" disabled={saving}>
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="input-group">
              ...
              <textarea className="input min-h-[60px]" value={infoForm.description}
            ...
            <ModalGrid>
              <div className="input-group">
                ...
                <input className="input" value={infoForm.user_id}
                  onChange={e => infoUpd('user_id', e.target.value)} placeholder="Optional" />
              </div>
              <div className="input-group">
                <label className="input-label">Email</label>
                <input className="input" value={infoForm.target_email}
